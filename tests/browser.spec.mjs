...
test('all cases remain accessible with JavaScript disabled',async({browser})=>{
 const context=await browser.newContext({javaScriptEnabled:false});const page=await context.newPage();await page.goto('http://127.0.0.1:4173/');await expect(page.locator('.piece-link')).toHaveCount(8);await page.locator('[data-project="reggio-projects"]').click();await expect(page.locator('h1')).toHaveText('Reggio projects');await page.getByRole('link',{name:'Back to collection',exact:false}).first().click();await expect(page).toHaveURL(/\/#work$/);await context.close();
});
